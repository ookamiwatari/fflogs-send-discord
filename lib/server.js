...
function getReportList() {
	...
	targetList.users.forEach(function(userName) {
		...
		request('GET', url).then(function (response) {
			...
			console.log('getReportList - err', err);
		});
	});
	...
	for(var report in targetList.reports) {
		(function(n) {
			...
			request('GET', url).then(function (response) {
				...
				console.log('getFightList - err', err);
			});
		})(report);
	}
	...
	Promise.resolve().then(function() {
		...
		return new Promise(function(resolve, reject) {
			...
			nightmare
				...
				.wait('svg')
				.evaluate(getPerfs)
				.then(function (nightmareResult) {
				
					resolve(JSON.parse(nightmareResult));
		...
		return new Promise(function(resolve, reject) {
			...
			nightmare
				.goto('https://ja.fflogs.com/reports/' + fight.fightId + '#fight=' + fight.id + '&type=deaths')
				.wait('svg')
				.evaluate(getDeaths)
				.then(function (nightmareResult) {

					resolve([result, JSON.parse(nightmareResult)]);
		...
		var message = headerMessage + createResultMessage(result[0], result[1]);
...
function createResultMessage(fflogsPerfsResponce, fflogsDeathsResponce ) {

	var msg = '```\n';
	for(var name in fflogsPerfsResponce.player) {

		if(fflogsPerfsResponce.player[name].perf) {
			msg += fflogsPerfsResponce.player[name].perf;
			msg += '% ';
		}
		...
		msg += ' (';
		msg += fflogsPerfsResponce.player[name].job;
		msg += ') ';

		msg += fflogsPerfsResponce.player[name].dps;

		if(fflogsDeathsResponce[name] == 1) {
			msg += ' <1death>';
		} else if (fflogsDeathsResponce[name] > 1) {
			msg += ' <' + fflogsDeathsResponce[name] + 'deaths>'
		}
	...
	if(fflogsPerfsResponce.total.perf) {
		msg += fflogsPerfsResponce.total.perf;
		msg += '% ';
	}

	if(fflogsPerfsResponce.total.dps) {
		msg += fflogsPerfsResponce.total.dps;
	}
...
var getPerfs = function() {
	...
	for (var i = 0, size = Array.from(document.querySelectorAll('.table-icon')).length; i < size; i++) {
		...
		if(Array.from(document.querySelectorAll('.table-icon'))[i]) {
			result.player[name]['job'] = Array.from(document.querySelectorAll('.table-icon'))[i].attributes[1].value.match(/actor-sprite-(.*)/)[1];
		}
...
var getDeaths = function() {
	...
	var result = {};
	// 各々の値を取得
	for (var i = 0, size = Array.from(document.querySelectorAll('.main-table-name')).length; i < size; i++) {
